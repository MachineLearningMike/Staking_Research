...
contract CompoundBurnActivity {
    ...
    function changePrincipal(address user, uint amount, bool CreditNotDebit) internal {
        {
            ...
            activity = _safeSubtract(activity * p1 /q1, principals[user] * p2 / q2);
    ...
    function pending(address user) public view returns (uint pending) {
        ...
        if (blocks > 0) {
            ...
            pending = _safeSubtract(principals[user], principals[user] * p / q);
            // pending = _safeSubtract(principals[user], IntegralMath.mulDivC(principals[user], p, q));
        }
    }
    ...
    function totalPending() public view returns (uint pending) {
        uint blocks = block.number - initBlock - lastBlock;
        (uint p, uint q) = analyticMath.pow(scale - rate, scale, blocks, cycle);
        pending = _safeSubtract(totalPrincipal, activity * p / q);
        // pending = _safeSubtract(totalPrincipal, IntegralMath.mulDivF(activity, p, q));
    }
