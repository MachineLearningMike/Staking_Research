...
// principals[user] += principals[user] * ( ( 1 + rewardRate ) ** blocks_passed - 1 )
//=====================================================================================

contract CompoundBurnPendency {
    ...
    uint public constant alpha = 12; // 1 for TypeC. 1e22;
    uint initBlock;
    uint lastBlock;
    ...
    function update() public {
        ...
        if (blocks > 0) {
            uint net = totalPrincipal - pendency;
            (uint p, uint q) = analyticMath.pow(scale - rate, scale, blocks, cycle);
            uint pending = _safeSubtract(net, net * p / q);
            // uint pending = net - IntegralMath.mulDivC(net, p, q);
            pendency += pending;
    ...
    function pending(address user) public view returns (uint pending) {
        ...
        if (blocks > 0) {
            (uint p, uint q) = analyticMath.pow(scale - rate, scale, blocks, cycle);
            pending = _safeSubtract(principals[user], principals[user] * p / q);    // mulDivC for larger balance
            // pending = principals[user] - IntegralMath.mulDivF(principals[user], p, q);    // mulDivC for larger balance
        }
    }
    ...
    function totalPending() public view returns (uint pending) {
        ...
        if (blocks > 0) {
            uint net = totalPrincipal - pendency;
            (uint p, uint q) = analyticMath.pow(scale - rate, scale, blocks, cycle);
            pending += _safeSubtract(net, net * p / q);
            // pending += (net - IntegralMath.mulDivF(net, p, q));
        }
    }
