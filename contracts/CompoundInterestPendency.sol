...
contract CompoundInterestPendency {
    ...
    function update() public {
        ...
        if (blocks > 0) {
            ...
            uint pending = _safeSubtract(net * p / q, net);
            // uint pending = IntegralMath.mulDivC(net, p, q) - net;
            pendency += pending;
    ...
    function pending(address user) public view returns (uint pending) {
        ...
        if (blocks > 0) {
            (uint p, uint q) = analyticMath.pow(scale + rate, scale, blocks, cycle);
            pending = _safeSubtract(principals[user] * p / q, principals[user]);
            // pending = IntegralMath.mulDivC(principals[user], p, q) - principals[user];
        }
    }
    ...
    function totalPending() public view returns (uint pending) {
        ...
        if (blocks > 0) {
            uint net = totalPrincipal + pendency;
            (uint p, uint q) = analyticMath.pow(scale + rate, scale, blocks, cycle);
            pending += _safeSubtract(net * p / q, net);
            // pending += (IntegralMath.mulDivC(net, p, q) - net);
        }
    }
