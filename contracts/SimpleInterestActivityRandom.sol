...
contract SimpleInterestActivityRandom {
    ...
    uint activity; uint totalReward; uint lastBlock;


    function changePrincipal(address user, uint amount, bool CreditNotDebit) internal {
        {
            activity -= principals[user] * users[user].lastBlock;
        }
    ...
    function totalPending() public view returns (uint pending) {
        ...
        // pending = (totalPrincipal * blocks - activity) * rate / scale / cycle;
        if (blocks % 2 == 0) {
            pending = IntegralMath.mulDivF((totalPrincipal * blocks - activity), rate, scale * cycle);
        } else {
            pending = IntegralMath.mulDivC((totalPrincipal * blocks - activity), rate, scale * cycle);
        }
    }
