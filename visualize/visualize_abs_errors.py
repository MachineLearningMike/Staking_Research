...
fig.suptitle("Absolute errors in {} Total Principal test mode".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    for ax in range(len(axTitles)):
        if (axTitles[ax].upper() == files[fileNo][0].upper()):
            axes[ax].plot(blockNo, collective_abs_error, label="$|totalBalance() - javascriptTruth|$", linewidth=1.0)
            axes[ax].plot(blockNo, marginal_abs_error, label="$|\sum_{u \in U} balance(u) - javascriptTrut|$", linewidth=1.0)
