...
fig.suptitle("Log absolute errors in {} Total Principal test mode".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    collective_abs_error = np.array([abs(a[0] - a[3]) for a in ar], dtype=np.float64)
    marginal_abs_error = np.array([abs(a[1] - a[3]) for a in ar], dtype=np.float64)
    blockNo = np.array([a[6] for a in ar], dtype=np.float64)

    for ax in range(len(axTitles)):
        if (axTitles[ax].upper() == files[fileNo][0].upper()):
            axes[ax].plot(blockNo, np.log(collective_abs_error), label="$log(|totalBalance() - javascriptTruth|)$", linewidth=1.0)
            axes[ax].plot(blockNo, np.log(marginal_abs_error), label="$log(|\sum_{u \in U} balance(u) - javascriptTrut|)$", linewidth=1.0)
