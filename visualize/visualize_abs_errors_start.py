...
fig.suptitle("Absolute errors during start period in {} Total Principal test mode".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    collective_abs_error = [abs(a[0] - a[3]) for a in ar]
    marginal_abs_error = [abs(a[1] - a[3]) for a in ar]
    blockNo = [a[6] for a in ar]

    span = 30
    for ax in range(len(axTitles)):
        if (axTitles[ax].upper() == files[fileNo][0].upper()):
            axes[ax].plot(blockNo[:span], collective_abs_error[:span], label="$|totalBalance() - javascriptTruth|$", linewidth=1.0)
            axes[ax].plot(blockNo[:span], marginal_abs_error[:span], label="$|\sum_{u \in U} balance(u) - javascriptTrut|$", linewidth=1.0)
