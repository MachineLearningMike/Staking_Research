...
fig.suptitle("Major measurements in {} Total Principal test mode".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    for ax in range(len(axTitles)):
        if (axTitles[ax].upper() == files[fileNo][0].upper()):
            axes[ax].plot(blockNo, collective, label="$totalBalance()$", linewidth=1.0)
            axes[ax].plot(blockNo, marginal, label="$\sum_{u \in U} balance(u)$", linewidth=1.0)
            axes[ax].plot(blockNo, javascriptTruth, label="$javascriptTrut$", linewidth=1.0)            

            # axes[ax].plot(blockNo, solidityTruth, label="$solidityTruth$", linewidth=1.0)
