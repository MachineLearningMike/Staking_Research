...
fig.suptitle("Logarithm of major measurements in {} Total Principal test mode".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    array = np.array(array, np.float64)     # precision is degraded a lot, for large numbers.
    ...
    array = np.array(array, dtype=np.longdouble)   # lose accuracy for large integers
    collective = array[:, 0]
    marginal = array[:, 1]
    solidityTruth = array[:, 2]
    javascriptTruth = array[:, 3]
    abs_error = array[:, 4]
    rel_error = array[:, 5]
    blockNo = array[:, 6]
    ...
    for ax in range(len(axTitles)):
        if (axTitles[ax].upper() == files[fileNo][0].upper()):
            ...
            axes[ax].plot(blockNo, np.log(collective), label="$log(totalBalance())$", linewidth=1.0)
            axes[ax].plot(blockNo, np.log(marginal), label="$log(\sum_{u \in U} balance(u))$", linewidth=1.0)
            # axes[ax].plot(blockNo, np.log(solidityTruth), label="$log(soldityTruth)$", linewidth=1.0)
            axes[ax].plot(blockNo, np.log(javascriptTruth), label="$log(javascriptTruth)$", linewidth=1.0)
