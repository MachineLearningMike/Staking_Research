...
fig.suptitle("Major measurements during start period in {} Total Principal test mode".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    for ax in range(len(axTitles)):
        if (axTitles[ax].upper() == files[fileNo][0].upper()):
            axes[ax].plot(blockNo[:span], collective[:span], label="$totalBalance()$", linewidth=1.0)
            axes[ax].plot(blockNo[:span], marginal[:span], label="$\sum_{u \in U} balance(u)$", linewidth=1.0)
            # axes[ax].plot(blockNo[:span], solidityTruth[:span], label="$solidityTruth$", linewidth=1.0)
            axes[ax].plot(blockNo[:span], javascriptTruth[:span], label="$javascriptTrut$", linewidth=1.0)
