...
fig.suptitle("Relative errors in {} Total Principal test mode".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    for ax in range(len(axTitles)):
        if (axTitles[ax].upper() == files[fileNo][0].upper()):
            axes[ax].plot(blockNo, collective_rel_error, label="$|totalBalance() - javascriptTruth| / |javascriptTruth|$", linewidth=1.0)
            axes[ax].plot(blockNo, marginal_rel_error, label="$|\sum_{u \in U} balance(u) - javascriptTrut| / |javascriptTruth|$", linewidth=1.0)
            text = "average: {:.2E}, {:.2E}".format(np.average(collective_rel_error), np.average(marginal_rel_error))
            axes[ax].text(0.4, 0.7, text, horizontalalignment='center', verticalalignment='center', transform=axes[ax].transAxes)
