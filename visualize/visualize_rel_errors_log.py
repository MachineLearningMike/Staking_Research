...
fig.suptitle("Log relative errors in {} Total Principal test mode".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    for ax in range(len(axTitles)):
        if (axTitles[ax].upper() == files[fileNo][0].upper()):
            axes[ax].plot(blockNo, np.log(collective_rel_error), label="$log(|totalBalance() - javascriptTruth| / |javascriptTruth|)$", linewidth=1.0)
            axes[ax].plot(blockNo, np.log(marginal_rel_error), label="$log(|\sum_{u \in U} balance(u) - javascriptTrut| / |javascriptTruth|)$", linewidth=1.0)
