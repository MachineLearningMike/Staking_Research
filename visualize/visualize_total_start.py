...
files = [   # title, filename, scale, ylim
    ...
    ("{} Interest pendency tracker".format(CS), "z_{}InterestPendency_total.txt".format(CS), 1e-27, 3.0e-21),
    ("{} Burn pendency tracker".format(CS), "z_{}BurnPendency_total.txt".format(CS), 1e-27, 2.0e-16),
    ("{} Interest activity tracker".format(CS), "z_{}InterestActivity_total.txt".format(CS), 1e-27, 2.9e-21),
    ("{} Burn activity tracker".format(CS), "z_{}BurnActivity_total.txt".format(CS), 1e-27, 2.2e-16),
...
fig.suptitle("TotalBalance() during start period in {} Total Principal test mode".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    totalBalance = [a[0] for a in ar]
    blockNo = [a[1] for a in ar]

    span = 200
    for ax in range(len(axTitles)):
        if (axTitles[ax].upper() == files[fileNo][0].upper()):
            axes[ax].plot(blockNo[:span], totalBalance[:span], label="$totalBalance()$", linewidth=1.0)
