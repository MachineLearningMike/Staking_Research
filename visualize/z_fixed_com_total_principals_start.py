...
fig.suptitle("TotalBalance during staring period in compound tasks in Fixed Total Principal test mode".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    for ax in range(len(axTitles)):
        if (files[fileNo][0].upper().find(axTitles[ax].upper()) >= 0):
            if (ax==0):
                axes[ax].plot(blockNo[:span], np.array(javascriptTruth[:span]), label="totalBalance = totalPrincipal + totalPending - regulating burn", linewidth=1.0)
                axes[ax].legend(loc="upper left")
            else:
                axes[ax].plot(blockNo[:span], np.array(javascriptTruth[:span]), label="totalBalance = totalPrincipal - totalPending + regulating mint", linewidth=1.0)
