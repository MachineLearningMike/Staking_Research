...
fig.suptitle("TotalBalance in compound tasks in Free Total Principal test mode".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    for ax in range(len(axTitles)):
        if (files[fileNo][0].upper().find(axTitles[ax].upper()) >= 0):
            if (ax==0):
                axes[ax].plot(blockNo, np.array(javascriptTruth), label="totalBalance = totalPrincipal + totalPending", linewidth=1.0)
