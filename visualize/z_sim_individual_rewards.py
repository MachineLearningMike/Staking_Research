...
fig.suptitle("Reward of individual user in simple tasks".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    for ax in range(len(axTitles)):
        if (files[fileNo][0].upper().find(axTitles[ax].upper()) >= 0):
            if (ax==0):
                axes[ax].plot(blockNo, np.array(marginal)/5, label="reward[Alice], or interest[Alice]", linewidth=1.0)
                axes[ax].legend(loc="upper left")
            else:
                axes[ax].plot(blockNo, np.array(marginal)/5, label="reward[Alice], or burn[Alice]", linewidth=1.0)
                axes[ax].legend(loc="upper left")
