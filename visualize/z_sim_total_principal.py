...
fig.suptitle("Total principals in simple tasks".format(FixedFree), fontsize=20, color = 'darkblue')
...
for fileNo in range(len(files)):
    ...
    totalPrincipal = [10**9/3 for a in ar]

    for ax in range(len(axTitles)):
        if (files[fileNo][0].upper().find(axTitles[ax].upper()) >= 0):
            if (ax==0):
                axes[ax].plot(blockNo, np.array(totalPrincipal), label="totalPrincipal", linewidth=1.0)
